 ...
 *	Assignment: Repeat Names Ignore Case
 *	Project Description: This program is 
 *  intended to display all the repeated 
 *  names in the name array to the console 
 *  if the names are exactly equal in case 
 *  to each other
 * 
 **/
public class Display_Strings_IgnoreCase {
	...
	public static void main(String[] args){
		for(int i = 0; i < names.length; i++){
			...
			for(int n = 0; n < names.length; n++){
				if(n == i){
					break;
				}else if(comp_name.equalsIgnoreCase(names[n])){
					println(comp_name);
					break;
				}// End of same check
